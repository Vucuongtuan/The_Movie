   ...
   useEffect(() =>{
    ...
    const handleLogin = async() => {
        ...
        const getSignInWithEmailAndPassword = async() =>{
            try{
                await signInWithEmailAndPassword(auth,emailChange,passwordChange).then((userSignIn)=>{
                    toast.success('Đăng nhập thành công !!! , Hello ' + userSignIn.user.email)
                    setTimeout(()=>{
                        const locData =  {
    ...
    return ( 
        <Offcanvas show={showOffcanvas} onHide={handleClose}  key='end'  placement='end' name='end' style={{backgroundColor:'rgb(32,33,36)' ,color:'white'}}>
        <Offcanvas.Header closeButton>
        </Offcanvas.Header>
        <Offcanvas.Body>
        <div className="form-account">
            <h2 className="grow text-center h-[40px]">
                {/* Đăng nhập */}
                </h2>
            <div className='w-[100%]'>
        ...
        />
                ...
                <FontAwesomeIcon icon={viewPass=== false ? faEye : faEyeSlash } onClick={()=>setViewPass(!viewPass)}  className='text-white float-right absolute right-[50px] mt-[40px] z-50'/>
        <input 
    ...
    className={emailChange && passwordChange ? 'active bg-slate-950 text-white px-5 py-2 rounded-md font-semibold text-lg':'bg-slate-600 text-white px-5 py-2 rounded-md font-semibold text-lg '}>
        Đăng nhập</button>
                </div>
                <Link to="/register" align='center'>Go to register</Link>
        </div>
        {/* <TableUser/> */}
        <ToastContainer
position="top-right"
