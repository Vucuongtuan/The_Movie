  ...
  useEffect(() => {
    const getData = async () => {
      ...
      try {
        ...
        setLinkMovie([
          res?.data.data?.item?.episodes[0]?.server_data[tap - 1 || 0]
            .link_embed,
          'link_embed',
        ]);
      } catch (err) {
  ...
  return (
    <main className=' m-auto mb-4 px-[8rem] 2xl:px-[5rem] xl:px-[4rem] lg:px-[3rem] md:px-[1rem]'>
      ...
      {isLoading ? (
        ...
        // <>
        //   {dataMovie?.data?.item?.episodes[0]?.server_data.slug === '' ? (
        //     <iframe
        //       src={dataMovie?.data?.item.trailer_url
        //         .replace('www.youtube.com', 'www.youtube.com/embed')
        //         .replace('/watch?v=', '/')}
        ...
        <>
          <section className='w-full h-auto'>
            ...
            <div className='w-full py-1 mt-2 flex justify-end'>
             ...
             ${
               linkMovie[0] ===
               dataMovie?.data?.item?.episodes[0]?.server_data[tap - 1 || 0]
                 ?.link_embed
              ...
              ${
                linkMovie[0] ===
                dataMovie?.data?.item?.episodes[0]?.server_data[tap - 1]
                  ?.link_m3u8
                  ? 'cursor-no-drop bg-red-600 '
                  : 'hover:bg-white hover:text-black'
              }
          ...
          <section className='w-full h-auto'>
            ...
            <div className='w-full py-4 flex flex-wrap overflow-y-scroll max-h-[800px]'>
              ...
              '' ? (
                ...
                dataMovie?.data?.item?.episodes[0]?.server_data?.map(
                  (movie) => {
